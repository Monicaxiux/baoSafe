<template>
    <Search :userType="1" :data="eiInfo" :select="selectUser" :changeTime="changeTime"></Search>
    <Table :tableType="1" :loading="loading" :tableData="tableData"></Table>
...
import { selectRecord, selectRecordProject } from '@/api/areas'
import { record, EiInfo } from '@/types'
import Pagination from '@/components/Pagination.vue'//分页组件
import { reactive, ref } from 'vue';
const quer = reactive(new record)
...
const selectUser = () => {
    loading.value = true
    selectRecordProject(eiInfo).then((res: any) => {
        loading.value = false
        //将用户信息列表数据传入子组件
