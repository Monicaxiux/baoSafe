<template>
    <Search :userType="2" :data="eiInfo" :select="selectUser" :changeTime="changeTime"></Search>
    <Table :tableType="2" :loading="loading" :tableData="tableData"></Table>
...
import { selectRecord } from '@/api/areas'
import { record, recordProject, EiInfo } from '@/types'
import Pagination from '@/components/Pagination.vue'//分页组件
import { reactive, ref } from 'vue';
const quer = reactive(new recordProject)
...
const selectUser = () => {
    loading.value = true
    selectRecord(eiInfo).then((res: any) => {
        loading.value = false
        //将用户信息列表数据传入子组件
